...
namespace Moths.ModularBuilder.Editor.Controllers
{
    public class SceneGrid : IGrid
    {
        ...
        public int CurrentHeightIndex
        {
            get
            {
                return Mathf.FloorToInt(totalHeightIndex % heightPerLevel);
            }
            set
            {
                totalHeightIndex = CurrentLevelIndex * heightPerLevel + value;
            }
        }
        ...
        public int CurrentLevelIndex
        {
            get
            {
                if (heightPerLevel == 0) return 0;
                return Mathf.FloorToInt(totalHeightIndex / heightPerLevel);
            }
            set
            {
                totalHeightIndex = value * heightPerLevel;
            }
        }
